...
import com.ksy.camera.publish.Constants;
import com.ksy.camera.publish.KSYAvcNaluType;
import com.ksy.camera.publish.KSYUtils;
...
public class KSYFlv {
	 ...
	 * 
	 * @param h
	 *            the handler to send the message.
	 */
	...
	public void writeVideoSample(final ByteBuffer bb, MediaCodec.BufferInfo bi) throws Exception {
		...
		while (bb.position() < bi.size) {
			...
			if (nal_unit_type == KSYAvcNaluType.SPS || nal_unit_type == KSYAvcNaluType.PPS) {
				Log.i(Constants.TAG, String.format("annexb demux %dB, pts=%d, frame=%dB, nalu=%d", bi.size, pts, frame.size, nal_unit_type));
			}
	...
	private void write_h264_sps_pps(int dts, int pts) {
		...
		Log.i(Constants.TAG, String.format("flv: h264 sps/pps sent, sps=%dB, pps=%dB", h264_sps.length, h264_pps.length));
	}
	...
	private void rtmp_write_packet(int type, int dts, int frame_type, int avc_aac_type, KSYFlvFrameBytes tag) {
		...
		if (handler == null) {
			Log.w(Constants.TAG, "flv: drop frame for handler not ready.");
			return;
		}
