...
public class KSYHttpFlvClient {
	...
	private void disconnect() {
		...
		Log.i(Constants.TAG, "worker: disconnect SRS ok.");
	}
	...
	private void reconnect() throws Exception {
		...
		Log.i(Constants.TAG, String.format("worker: connect to SRS by url=%s", url));
		conn.setDoOutput(true);
		// conn.setChunkedStreamingMode(0);
		// conn.setRequestProperty("Content-Type", "application/octet-stream");
		bos = new BufferedOutputStream(conn.getOutputStream());
		Log.i(Constants.TAG, String.format("worker: muxer opened, url=%s", url));
